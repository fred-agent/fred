                ...
                backgroundColor:
                  index % 2 === 0
                    ? theme.palette.mode === "light"
                      ...
                      : theme.palette.grey[800] // Even rows
                    : theme.palette.mode === "light"
                      ? theme.palette.common.white
                      : theme.palette.grey[900], // Odd rows
              }}
            >
...
export const WorkloadCard = ({
  ...
  const handleNavigateToScoreDetail = () => {
    navigate(`/score/${cluster.alias}/${resource.namespace}/${resource.name}?kind=${resource.kind}`, {
      state: { score },
    });
  };
  const handleNavigateToFacts = () => {
    navigate(
      `/facts-workload?cluster=${cluster.fullname}&namespace=${resource.namespace}&workload=${resource.name}&kind=${resource.kind}`,
      { state: { score } },
    );
  };
        ...
        >
          ...
          >
            ...
            <Grid2 size={{ xs: 2 }} display="flex" justifyContent="flex-end">
              <IconButton sx={{ position: "relative" }} onClick={() => handleNavigateToScoreDetail()}>
                <CropFreeIcon />
              </IconButton>
            </Grid2>
          </Grid2>
