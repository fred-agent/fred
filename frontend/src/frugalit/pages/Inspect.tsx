...
const Inspect = () => {
  ...
  const getNamespacesFootprint = (footprint: ClusterConsumption) => {
    ...
    return selectedNamespaces
      .map((selectedNamespace: string) => footprint.details.find((detail) => detail.name === selectedNamespace))
      .filter((detail) => detail !== undefined)
      .map((detail) => ({
        alias: detail.name,
        fullname: "toto",
        value: detail.values.map((value) => value).reduce((sum, value) => sum + value, 0),
      }));
  };
