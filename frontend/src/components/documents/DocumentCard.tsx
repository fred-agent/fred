...
export const DocumentCard = ({
  ...
  const handleDisplayMetadata = async () => {
    try {
      const response = await retrieveMetadata({
        document_uid: fileId,
      }).unwrap();
  ...
  return (
    ...
    >
      <Box display="flex" flexDirection="row" alignItems="center" width="100%">
        ...
        {agent_name && <Box sx={{ marginRight: "10%" }}>{getAgentBadge(agent_name)}</Box>}
        ...
        {date_added_to_kb && (
          <Tooltip title="Date d'ajout à la base documentaire">
            <Box
              ...
              sx={{
                backgroundColor:
                  theme.palette.mode === "dark" ? theme.palette.text.disabled : theme.palette.chip.mediumGrey,
                borderRadius: "15px",
                padding: "5px",
                fontSize: "0.75rem",
                height: "24px",
        ...
        {size !== "small" && (
          <Box sx={{ marginLeft: 6, marginRight: "40px" }}>
            {" "}
            {/* Adjusted position */}
            <Tooltip
              enterDelay={500}
              title={isRetrievable ? "The document is retrievable" : "The document is not retrieable"}
            >
