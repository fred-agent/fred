...
export const OptimizeGainCard = (props: {
    date: Dayjs;
    data: SerieType[];
}) => {
    ...
    const getPercentage = (serieType: SerieType) => {
        if (serieType.series.length > 1) {
            return (getSumValue(serieType.series[0]) - getSumValue(serieType.series[1])) * 100 / getSumValue(serieType.series[1])
        } else {
            return 0
        }
    };
    ...
    const getIcon = (serieType: SerieType) => {
        ...
        if (getPercentage(serieType) > 0) {
            return <UpIcon sx={{ color: getCorrectColor(getPercentage(serieType)), fontSize: iconSize }} />
        } else if (getPercentage(serieType) === 0) {
            return <ViewStreamIcon sx={{ color: getCorrectColor(getPercentage(serieType)), fontSize: iconSize }} />
        } else {
            return <DownIcon sx={{ color: getCorrectColor(getPercentage(serieType)), fontSize: iconSize }} />
        }
    };
            ...
            <Paper sx={{ width: "100%", display: 'flex' }}>
                {isLargeScreen ? (
                    <><Grid2 container size={3} sx={{ justifyContent: 'center' }}>
                        <Grid2 size={12} p={3}
                               sx={{ display: 'flex', justifyContent: 'center', height: '6vh', alignItems: 'center' }}>
                            <InsightsIcon sx={{ width: '30px', height: '30px' }} />
                            <Typography
                            variant="h5"
                                sx={{ paddingLeft: 1 }}>Gain insights</Typography>
                        </Grid2>
                        <Divider sx={{
                            borderWidth: 1,
                        ...
                        <Grid2 size={12} sx={{ display: 'flex', justifyContent: 'center', height: '6vh', alignItems: 'center' }}>
                ...
                <Grid2 container size={isLargeScreen ? 9 : 12} sx={{ justifyContent: 'center' }}>
                    <Grid2 size={12}
                           sx={{ display: 'flex', justifyContent: 'center', height: '12vh', alignItems: 'center' }}>
                        {props.data?.length > 0 && (props.data.map((serieType, index) => (
                            <React.Fragment key={index}>
                                <Grid2 size={4} sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                                    <ImageComponent
                                        name={serieType.logos[0]}
                                        width="30px"
                                        height="30px"
                                    />
                                    <Grid2 sx={{ display: 'row', paddingLeft: 2, paddingRight: 2 }}>
                                        <Typography variant="h5" sx={{ color: getCorrectColor(getPercentage(serieType)), paddingLeft: 1 }}>
                                            {getPercentage(serieType).toFixed(2)} %
                                        </Typography>
                                        <Typography variant="h6" sx={{ paddingLeft: 1 }}>
                                            {getDiff(serieType) > 0 && ('+')} {getDiff(serieType)} {serieType.series.length > 0 && (serieType.series[0].unit)}
                                        </Typography>
                                    </Grid2>
                                    {getIcon(serieType)}
                                </Grid2>
