                ...
                <Grid2 container p={2}>
                    <Grid2 size={{xs: 12}} justifyContent="space-between" display="flex" alignItems="center">
                        <Grid2 size='auto' alignItems="center">
                            {props.data?.serieTypes?.length > 0 ? <ToggleButtonGroup
                                value={chartSerieType}
                                exclusive
                                onChange={handleChartSerieTypeChange}
                            >
                            ...
                            </ToggleButtonGroup> : <Skeleton animation="wave" width={"115px"} height={"30px"}
                        ...
                        {props.data?.serieTypes[0]?.series?.length > 1 ? (
                                ...
                                <Grid2 size={{xs: 12}} display="flex" alignItems="center" justifyContent="center">
                                    {chartSerieType.series.map((serie) => (
                                        <Grid2 container key={serie.name} alignItems="center">
                                            <HorizontalRuleIcon
                                                sx={{color: serie.color, ml: 1, fontSize: '5vh'}}></HorizontalRuleIcon>
                                            <Typography sx={{mr: 1}}>
                                                {serie.name} ({serie.seriePoints.length > 0 && serie.seriePoints[0]?.date ? dayjs(serie.seriePoints[0].date).format('DD MMM') : 'N/A'} - {serie.seriePoints.length > 0 && serie.seriePoints[serie.seriePoints.length - 1]?.date ? dayjs(serie.seriePoints[serie.seriePoints.length - 1].date).format('DD MMM YYYY') : 'N/A'})
                                            </Typography>
                                        </Grid2>
                                    ))}
                                </Grid2>
                            ...
                            chartSerieType?.series?.length > 0 && (
                                <Grid2 size={{xs: 12}} display="flex" alignItems="center" justifyContent="center">
                                    <Typography>{chartSerieType.series[0].name} : {getSumValue(chartSerieType.series[0]).toLocaleString()} {chartSerieType.series[0].unit}</Typography>
                                </Grid2>
                            )
                        )}
                            ...
                            <Select
                                ...
                                sx={{
                                   //  width: '5vw',
                                    ...
                                    '.MuiOutlinedInput-notchedOutline': { // This targets the inner outline border
                    ...
                    <Grid2  size={12} pt={1}>
                        <Box height={props.height}>
                            {chartSerieType ? <RiftChart
                                series={chartSerieType.series}
                                type={chartType}
                            >
                            </RiftChart> : <Skeleton animation="wave" width={"100%"} height={"100%"}/>}
                        </Box>
                    </Grid2>
                </Grid2>
            </Paper>
