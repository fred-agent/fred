...
async function fetchResourceDetails({
  ...
  const [summaryRes, essentialsRes, advancedRes, applicationRes, scoreRes, factsRes] = await Promise.all([
    getSummary({
      cluster: cluster,
      namespace: namespace,
...
export const ExplainContextProvider = (props: PropsWithChildren<{}>) => {
  ...
  const loadResource = async (clusterName: string, namespace: string, resourceName: string, resourceKind: string) => {
    ...
    const { summary, essentials, advanced, id, score, facts } = await fetchResourceDetails({
